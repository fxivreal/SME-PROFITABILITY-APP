-- Add support for additional production costs (transport, labor, packaging, etc.)
-- These are entered at batch completion time for accurate costing

ALTER TABLE production_batches
  ADD COLUMN IF NOT EXISTS additional_costs JSONB NOT NULL DEFAULT '[]'::jsonb,
  ADD COLUMN IF NOT EXISTS total_additional_cost DECIMAL(12,2) NOT NULL DEFAULT 0;

-- Update the complete_build function to include additional costs in final cost
...
BEGIN
  ...
  LOOP
    ...
    v_material_cost := v_material_cost + (v_rm.quantity_used * v_rm.cost_per_unit);
  END LOOP;

  v_total_cost := v_material_cost + v_batch.total_additional_cost;
  ...
  RETURN jsonb_build_object(
    'success', true,
    'total_material_cost', v_material_cost,
    'total_additional_cost', v_batch.total_additional_cost,
